$(document).ready(function () {
    ...
    jQuery(".dataTable").find('[class^= icon-]').tooltip();
    ...
    $(".row-form,.dialog,.loginBox,.block-fluid").find("input:checkbox, input:file").not(".skip").uniform();
...
jQuery(document).ready(function ($) {
    ...
    var arrTagP = $("#footer #cerrojo #cerr2 p").toArray();

    arrTagP.forEach(function (e) {

        var innerT = e.innerText;

        var indexC = innerT.indexOf("\u00A9");

        if (indexC != -1) {

            var subStr = innerT.substr(indexC + 2, innerT.length - (indexC + 2) - 1);

            var indexS = subStr.indexOf(" ");

            if (indexS != -1) {

                var year = innerT.substr(indexC + 2, indexS);

                e.innerText = innerT.replace(year, new Date().getFullYear());
