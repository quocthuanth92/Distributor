...
namespace CMSApplication.Areas.Administration.Models
{
    public class OrderModels
    {
        ...
        public SelectList PercentSelectList { get; set; }
        ...
        List<string> _percent = null;
        public void GetDataForDropdownList()
        {
            _percent = new List<string>();
            for (int i = 0; i <= 100; i++)
            {
                _percent.Add(i.ToString());
            }
            PercentSelectList = new SelectList(_percent, OrderPercent);
            SizeSelectList = new SelectList(_size, Size);
            TypeSelectList = new SelectList(_type, Type);
            StatusSelectList = new SelectList(_status, Status);
            ServiceTypeSelectlist = new SelectList(_service_type, ServiceType); 
            UserRepositry user_services = new UserRepositry();
            StaffSelectList = new List<SelectListItem>();
            UsersSelectlist = new SelectList(user_services.GetUsersByRole("Client"), "Id", "Email");
            var staffList = user_services.GetUsersByRole("Staff");
            if (StaffID != null)
            {
                foreach (var item in staffList)
                {
                    ...
                    s.Text = item.Email;
                    s.Value = item.Email;
                    if (StaffID.Contains(item.Email))
                    {
                        s.Selected = true;
                    }
                    StaffSelectList.Add(s);
                }
            }
        }
    }
