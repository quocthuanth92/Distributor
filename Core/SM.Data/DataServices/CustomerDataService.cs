...
namespace SM.Data
{
    ...
    /// </summary>
    public class CustomerService : EntityFrameworkService, ICustomerService
    {
        ...
        public void UpdateCustomer(CustomerManagement customer)
        {
        ...
        /// </summary>
        /// <param name="customer"></param>
        public void CreateCustomer(CustomerManagement customer)
        {
            dbConnection.CustomerManagements.Add(customer);
        }
        ...
        /// <returns></returns>
        public CustomerManagement GetCustomer(string customerCode)
        {
            CustomerManagement customer = dbConnection.CustomerManagements.Where(c => c.CustomerCode == customerCode).FirstOrDefault();
            return customer;
        }
        ...
        /// <returns></returns>
        public List<CustomerManagement> GetCustomers(int currentPageNumber, int pageSize, string sortExpression, string sortDirection, out int totalRows)
        {
            totalRows = 0;

            sortExpression = sortExpression + " " + sortDirection;

            totalRows = dbConnection.CustomerManagements.Count();

            List<CustomerManagement> customers = dbConnection.CustomerManagements.OrderBy(sortExpression).Skip((currentPageNumber - 1) * pageSize).Take(pageSize).ToList();
